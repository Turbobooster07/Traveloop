...
const BuildItinerary = () => {
  ...
  const addCity = (sectionId) => {
    setSections(prev => prev.map(s =>
      s.id === sectionId ? { ...s, cities: [...s.cities, createCity()] } : s
    ));
  };
  ...
  const updateCity = (sectionId, cityId, field, value) => {
    setSections(prev => prev.map(s =>
      s.id === sectionId ? {
        ...s,
        cities: s.cities.map(c => c.id === cityId ? { ...c, [field]: value } : c)
      } : s
    ));
  };
  ...
  const addStop = (sectionId, cityId) => {
    setSections(prev => prev.map(s =>
      s.id === sectionId ? {
        ...s,
        cities: s.cities.map(c =>
          c.id === cityId ? { ...c, stops: [...c.stops, createStop()] } : c
        )
      } : s
    ));
  };
      ...
      s.id === sectionId ? {
        ...
        cities: s.cities.map(c =>
          c.id === cityId ? {
            ...c,
            stops: c.stops.map(st =>
              st.id === stopId ? { ...st, [field]: value } : st
            )
          } : c
        )
      } : s
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '860px' }}>
          {sections.map((section, index) => (
            <div key={section.id} style={{
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <textarea rows={3} value={section.description}
              ...
              <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: '24px' }}>
                <div style={{ flex: '1 1 180px' }}>
                  ...
                  <input type="date" value={section.from_date}
                    onChange={e => updateSection(section.id, 'from_date', e.target.value)} style={inputStyle} />
                </div>
                <div style={{ flex: '1 1 180px' }}>
                  <label style={labelStyle}>To</label>
                  <input type="date" value={section.to_date}
                    onChange={e => updateSection(section.id, 'to_date', e.target.value)} style={inputStyle} />
                </div>
                <div style={{ flex: '1 1 180px' }}>
          ...
          <button onClick={addSection}
            style={{
              ...
              width: '100%', maxWidth: '860px', padding: '20px',
              background: 'var(--card-bg)', backdropFilter: 'blur(20px)',
              WebkitBackdropFilter: 'blur(20px)',
              border: '2px dashed var(--border-medium)', borderRadius: '28px',
          ...
          <div style={{ display: 'flex', gap: '16px', maxWidth: '860px' }}>
            <button onClick={() => navigate('/dashboard', { state: { user } })}
              style={{
                flex: 1, padding: '16px', background: 'var(--card-bg)',
                backdropFilter: 'blur(20px)', border: '1px solid var(--border-medium)',
                borderRadius: '16px', color: 'var(--text-muted)', fontSize: '16px',
            ...
            <button onClick={handleSave} disabled={saving || saved}
              className="login-btn"
              style={{
                flex: 2, padding: '16px', fontSize: '17px', borderRadius: '16px',
                boxShadow: 'var(--cta-shadow)',
